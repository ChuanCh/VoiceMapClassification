function [egg, Qci, Qsi, maxDegg, Ic, ampl] = synthEGGfromFile(filename, points, periods)
...
%  - Qsi is the quotient of speed   by integration
...
Qci = zeros(nClusters,1);
Qsi = zeros(nClusters,1);
maxDegg = zeros(nClusters,1);
Ic = zeros(nClusters,1);

for n = 1:nClusters
   [eggN, tStruct] = synthEGGfromArrays(dLevels(n,:), phases(n,:), nDeltas, points, periods);
   %[eggN, tStruct] = synthEGGfromArraysSigma(dLevels(n,:), phases(n,:), nDeltas, points, periods);
   waves(n,:) = eggN;
   Qci(n) = tStruct.qC;
   %Qsi(n) = tStruct.qS;
