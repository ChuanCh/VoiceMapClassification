...
log_data = [];
    ...
    for i = 1:length(DIR)
        ...
        subject_name = DIR(i).name;
        s = fullfile(DIR(1).folder,DIR(i).name); 
        Folders = dir(s);
        n = 0;
            switch type
                case 'vrp'
                ...
                case 'log'
                for iCount = 1:length(Folders)
                    name = Folders(iCount).name;
                    if endsWith(name, 'aiff') && contains(name,'VRP')
...
%     metricsRep(:,5) = log10(metricsRep(:,5));
    ...
    [idx, C] = kmeans(metricsStd, k, 'Display','final','OnlinePhase', 'on','Replicates', 30, 'MaxIter',100000);
        ...
        for n = 1:length(spl)
            for m = 1:length(midi)        
                ...
                if ~isempty(a)
                    ...
                    cluster_m(4:9) = mean(log_range(a,[5 6 7 9 11 12]), 1);
